...
export class WorkflowCommandBus {
  ...
  changeNodeVersion(id: string, definition: Definition): { removedEdges: Edge[]; removedOutputs: Array<{ node_id: string; port: string }>; removedParameters: string[]; modelCleared: boolean } {
    const node = this.store.nodes().find((item) => item.id === id);
    if (!node) return { removedEdges: [], removedOutputs: [], removedParameters: [], modelCleared: false };
    ...
    const modelCleared = Boolean(node.model_binding?.model_version_id && !definition.default_model_version_id);
    this.commit(() => { this.store.nodes.update((items) => items.map((item) => item.id === id ? { ...item, node_version: definition.version, definition, parameters, ...(modelCleared ? { model_binding: null } : {}) } : item)); this.store.edges.update((items) => items.filter((edge) => !removedEdges.includes(edge))); this.store.outputs.update((items) => items.filter((output) => !removedOutputs.includes(output))); });
    return { removedEdges, removedOutputs, removedParameters, modelCleared };
  }
