...
import { AngularPlugin, Presets as AngularPresets, NodeComponent } from 'rete-angular-plugin/21';
...
export class ReteWorkflowAdapter {
  ...
  constructor(
    ...
    private readonly commands: WorkflowCommandBus,
    private readonly store: WorkflowEditorStore,
    private readonly renderers: NodeRendererRegistry,
  ) {}

  async mount(
    host: HTMLDivElement,
    snapshot: { nodes: EditorNode[]; edges: Edge[] },
    options: { editable?: boolean; onNodePicked?: (id: string) => void } = {},
  ): Promise<void> {
    if (this.editor && this.host === host) {
      this.editable = options.editable ?? this.editable;
      await this.syncIncremental(snapshot);
      return;
    }
    ...
    this.area = new AreaPlugin(host);
    this.selection = AreaExtensions.selector();
    AreaExtensions.selectableNodes(this.area, this.selection, {
    ...
    render.addPreset(
      AngularPresets.classic.setup({
        customize: {
          node: (context: any) =>
            this.renderers.resolve(this.rendererKey(context)) ?? NodeComponent,
        },
      }) as any,
    );
    ...
    try {
      for (const node of snapshot.nodes) {
        ...
        await this.addNode(node, generation);
      }
      for (const edge of snapshot.edges) {
        if (!this.isActiveMount(generation, host)) return;
        await this.addConnection(edge, generation);
      }
      ...
      if (this.isActiveMount(generation, host)) this.hydrating = wasHydrating;
    }
    if (!this.isActiveMount(generation, host)) return;
  ...
  setNodePickedHandler(handler?: (id: string) => void): void {
    this.nodePickedHandler = handler;
  }

  unmount(host: HTMLDivElement): void {
    if (this.host === host) {
  ...
  detachHost(host: HTMLDivElement): void {
    this.unmount(host);
  }

  async sync(snapshot: { nodes: EditorNode[]; edges: Edge[] }): Promise<void> {
    await this.syncIncremental(snapshot);
  }
  ...
  get areaPlugin(): AreaPlugin<any, any> | undefined {
    return this.area;
  }

  get editorInstance(): NodeEditor<any> | undefined {
    return this.editor;
  }

  get synchronizationProtected(): boolean {
    return this.hydrating;
  }

  get nodeCount(): number {
    return this.reteNodes.size;
  }
  ...
  async addNode(item: EditorNode, expectedGeneration = this.mountGeneration): Promise<void> {
    ...
    if (!editor || !area || this.reteNodes.has(item.id)) return;
    const node = new ClassicPreset.Node(item.definition?.node_name ?? item.node_code);
    for (const port of item.definition?.input_ports ?? []) {
      node.addInput(
        port.key,
        new ClassicPreset.Input(new ClassicPreset.Socket(port.data_type), port.label, Boolean(port.required)),
      );
    }
    for (const port of item.definition?.output_ports ?? []) {
      node.addOutput(
        port.key,
        new ClassicPreset.Output(new ClassicPreset.Socket(port.data_type), port.label),
      );
    }
    ...
    await area.translate(node.id, { x: item.x, y: item.y });
  }
  ...
  private async syncIncremental(snapshot: { nodes: EditorNode[]; edges: Edge[] }): Promise<void> {
    ...
    this.hydrating = true;
    try {
      const initialEmpty = this.reteNodes.size === 0 && snapshot.nodes.length > 0;
      const wanted = new Map(snapshot.nodes.map((node) => [node.id, node]));
      for (const id of [...this.reteNodes.keys()]) {
        if (!wanted.has(id)) await this.removeNode(id);
      }
      for (const node of snapshot.nodes) {
      ...
      for (const connection of [...this.editor.getConnections()]) {
        const edge: Edge = {
          source: { node_id: String(connection.source), port: String(connection.sourceOutput) },
          target: { node_id: String(connection.target), port: String(connection.targetInput) },
        };
      ...
      if (
        ...
        this.area &&
        this.editor &&
        typeof this.editor.getNodes === 'function' &&
        this.editor.getNodes().length > 0
      ) {
        await AreaExtensions.zoomAt(this.area, this.editor.getNodes());
      }
    } finally {
  ...
  async select(id: string): Promise<void> {
    const node = this.reteNodes.get(id);
    if (!node || !this.selection || !this.area) return;
    const selectable = AreaExtensions.selectableNodes(this.area, this.selection, {
      accumulating: AreaExtensions.accumulateOnCtrl(),
    });
  ...
  private async addConnection(edge: Edge, expectedGeneration = this.mountGeneration): Promise<boolean> {
    const editor = this.editor;
    if (!editor || expectedGeneration !== this.mountGeneration) return false;
    const source = this.reteNodes.get(edge.source.node_id);
    const target = this.reteNodes.get(edge.target.node_id);
    if (!source || !target) return false;
    try {
      await editor.addConnection(new ClassicPreset.Connection(source, edge.source.port, target, edge.target.port));
      return expectedGeneration === this.mountGeneration && editor === this.editor;
    } catch {
      return false;
    }
  }
    ...
    if (this.area) {
      this.area.addPipe((context: any) => {
        ...
        if (context.type === 'nodepicked') {
          this.store.selectedId.set(String(context.data.id));
          onNodePicked?.(String(context.data.id));
        }
        if (context.type === 'nodetranslate') {
          this.commands.moveNode(
            String(context.data.id),
            Number(context.data.position?.x ?? 0),
            Number(context.data.position?.y ?? 0),
          );
        }
        return context;
      });
      ...
      this.editor.addPipe((context: any) => {
        ...
        if (context.type === 'connectioncreate') {
          this.commands.connect({
            source: { node_id: String(context.data.source), port: String(context.data.sourceOutput) },
            target: { node_id: String(context.data.target), port: String(context.data.targetInput) },
          });
        }
        if (context.type === 'connectionremove') {
          this.commands.deleteConnection({
            source: { node_id: String(context.data.source), port: String(context.data.sourceOutput) },
            target: { node_id: String(context.data.target), port: String(context.data.targetInput) },
          });
        }
  ...
  private isActiveMount(generation: number, host: HTMLDivElement): boolean {
    return generation === this.mountGeneration && host === this.host;
  }
